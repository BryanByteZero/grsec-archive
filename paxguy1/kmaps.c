...
static unsigned int dump_pt(int devmem, unsigned int pte, unsigned int va)
{
  unsigned int *pt, i;
...
static unsigned long long dump_pt_pae(int devmem, unsigned long long pte, unsigned long long va)
{
  ...
  if (LARGE(pte)) {
    printf("\t\t\t%spte: LRG %016llx %016llx\n", EXECUTABLE(pte) ? YELLOW : LIGHTYELLOW, pte, va);
    goto out;
  }
  ...
  for (i = 0; i < PER_PAGE(pte); ++i, va += PAGE_SIZE) {
    if (pt[i])
      printf("\t\t\t%spte: %03x %016llx %016llx\n", EXECUTABLE(pt[i]) ? YELLOW : LIGHTYELLOW, i, pt[i], va);
  }
...
static unsigned long long dump_pmd_pae(int devmem, unsigned long long pmd, unsigned long long va)
{
  unsigned long long *pt;
  unsigned int i;
  static unsigned long long buffer[PAGE_SIZE / sizeof(unsigned long long)];
  ...
  if (LARGE(pmd)) {
    printf("\t\t%spmd: LRG %016llx %016llx\n", EXECUTABLE(pmd) ? GREEN : LIGHTGREEN, pmd, va);
    goto out;
  }
  ...
  for (i = 0; i < PER_PAGE(pmd); ++i) {
    if (pt[i])
      printf("\t\t%spmd: %03x %016llx %016llx\n", EXECUTABLE(pt[i]) ? GREEN : LIGHTGREEN, i, pt[i], va);
    va = dump_pt_pae(devmem, pt[i], va);
  }
...
#ifdef __x86_64__
static unsigned long long dump_pud_pae(int devmem, unsigned long long pud, unsigned long long va)
{
  unsigned long long *pt;
  unsigned int i;
  static unsigned long long buffer[PAGE_SIZE / sizeof(unsigned long long)];
  ...
  for (i = 0; i < PER_PAGE(pud); ++i) {
    if (pt[i])
      printf("\t%spud: %03x %016llx %016llx\n", EXECUTABLE(pt[i]) ? BLUE : LIGHTBLUE, i, pt[i], va);
    va = dump_pmd_pae(devmem, pt[i], va);
  }
...
static void dump_pgd(int devmem, unsigned int pgd)
{
...
static void dump_pgd_pae(int devmem, unsigned long long pgd, unsigned int pgd_size)
{
  ...
  for (i = 0; i < pgd_size; ++i) {
    if (pt[i])
      printf("%spgd: %03x %016llx %016llx\n", EXECUTABLE(pt[i]) ? RED : LIGHTRED, i, pt[i], va);
#ifdef __i386__
    va = dump_pmd_pae(devmem, pt[i], va);
#else
    va = CANONICALIZE(dump_pud_pae(devmem, pt[i], va));
#endif
  }
...
int main(int argc, char *argv[])
{
  ...
  if (1024 == pgd_size)
    ...
    dump_pgd_pae(devmem, pgd, pgd_size);
